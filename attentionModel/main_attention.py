...
def visualize_doc_all_extractions(doc_attention, doc_text, id):
    fig, ax = plt.subplots()
    ...
    plt.tight_layout()
    plt.show()
    plt.savefig(f'./visualizations/pen/attPenalization{id}.png')


# visualize all joined attention extractions for batch documents
def visualize_joined_att(attention, docs_text, id):
    ...
    for doc_att, doc_text in zip(norm_att, docs_text):
        ...
        plt.tight_layout()
        plt.show()
        plt.savefig(f'./visualizations/pen/attPenalizationJoined_{id}.png')
        id += 1
...
def visualize(batch, attention, vocab):
    ...
    if config['visualize_all_extractions']:
        ...
        for doc_attention, doc_text in zip(attention, docs_text):
            visualize_doc_all_extractions(doc_attention, doc_text, id_batch*len(batch) + id)
            id += 1
    if config['visualize_joined_extractions']:
        visualize_joined_att(attention, docs_text, id_batch*len(batch))
    id_batch += 1
